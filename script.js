...
/* ═══════════════════════════════════════════════════════════
   ALBUM VIEWER — Open / Close / Music / Build Grid
   ═══════════════════════════════════════════════════════════ */
(function initAlbumViewer() {
  ...
  // Gather all open buttons
  document.querySelectorAll(".album-open-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const idx = +btn.dataset.album;
      openAlbum(idx);
    });
  });

  // Album card click also opens
  document.querySelectorAll(".album-card").forEach(card => {
    card.addEventListener("click", () => {
      const idx = +card.dataset.album;
      openAlbum(idx);
    });
  });
  ...
  function openAlbum(idx) {
    ...
    // Update header
    badgeEl.textContent = album.badge;
    ...
    // Pause global music
    if (globalMusic && !globalMusic.paused) globalMusic.pause();
    if (musicPlayer) musicPlayer.classList.add("hidden-player");

    // Start album music
    startAlbumMusic(album.music);
  }
  ...
  function closeAlbum() {
    ...
    // Restore global music player visibility
  ...
  function buildMasonry(photos) {
    ...
    photos.forEach((photo, i) => {
      ...
      img.addEventListener("error", () => item.classList.add("loaded"));   // Show even if broken
  ...
  /* ── Album Music Controls ── */
  function startAlbumMusic(src) {
    if (!albumMusic || !src) return;
    albumMusic.src  = src;
    albumMusic.loop = true;
    albumMusic.volume = 0.5;
    albumMusic.play().then(() => {
      albumPlaying = true;
      updateMusicUI(true);
    }).catch(() => {
      albumPlaying = false;
  ...
  function updateMusicUI(playing) {
    if (!albumIconPlay || !albumIconMute) return;
    albumIconPlay.style.display = playing ? "none"  : "block";
    albumIconMute.style.display = playing ? "block" : "none";
  }

  if (albumMusicBtn) {
    albumMusicBtn.addEventListener("click", () => {
  ...
  /* ── Expose openAlbum for lightbox ── */
  window._albumGetPhotos = () => currentAlbum !== null ? ALBUM_DATA[currentAlbum].photos : [];
})();
